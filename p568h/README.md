...
## Solution:
### Intuitive Approach
Starting from each city, when moving to the next week, add the days when there is connection. In a DFS fashion, enumerate every path and find out the max value.

### Dynamic Programming Approach
Let DP variable `dp[i][k]` be the max vacation day start from city i in week k, where `i = [0, N), k = [0, K)`.

The key for this problem is *the max vacation days starting from city i on week k doesn't depend on week k-1, but on week k+1 and of course subject to connectivity constraints*. This means even we have `dp[.][k]` on week k, after seeing new information days[k+1] we need to update `dp[.][k]`, but not the other way. Hence we will propagate the dp matrix backward from day K-1 to day 0.
