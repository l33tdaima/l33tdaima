...
class Solution {
    int recMaxCoin(vector<int>& coins, vector<vector<int>>& memo, int l, int r) {
        ...
        int ret = 0;
        for (int i = l + 1; i < r; ++i) {
            ret = max(ret, coins[l] * coins[i] * coins[r]
                         + recMaxCoin(coins, memo, l, i)
                         + recMaxCoin(coins, memo, i, r));
        }
        memo[l][r] = ret;
        return ret;
    }
        ...
        for (int range = 2; range < n; ++range) {
            for (int left = 0; left < n - range; ++left) {
                ...
                for (int k = left + 1; k < right; ++k) {
                    dp[left][right] = max(
                        dp[left][right], 
                      + coins[left] * coins[k] * coins[right]
