...
public:
    // Soluion 1
    string boldWordsSlow(vector<string>& words, string S) {
        unordered_set<string> dict;
        for (auto& w: words) {
            dict.emplace(w);
        }
        vector<pair<int, int>> boldRanges;
        for (int i = 0; i < S.length(); ++i) {
            for (int j = i + 1; j <= S.length(); ++j) {
                if (dict.count(S.substr(i, j - i)) == 0) { continue; }
                int brlen = boldRanges.size();
                if (brlen && i <= boldRanges[brlen - 1].second + 1) {
                    if (j - 1 > boldRanges[brlen - 1].second) {
                        boldRanges[brlen - 1].second = j - 1;
                    }
                } else {
                    boldRanges.push_back(make_pair(i, j - 1));
                }
            }
        }
        if (boldRanges.size() == 0) { return S; }
        string ans;
        for (int i = 0, k = 0; i < S.length(); ++i) {
            if (k < boldRanges.size() && i == boldRanges[k].first) {
                ans += "<b>";
            }
            ans += S[i];
            if (k < boldRanges.size() && i == boldRanges[k].second) {
                ans += "</b>";
                k ++;
            }
        }
...
struct Test {
    ...
    void run() {
        Solution sol;
        string actual = sol.boldWords(words, S);
