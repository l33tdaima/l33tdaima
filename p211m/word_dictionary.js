...
WordDictionary.prototype.addWord = function (word) {
  ...
  for (let i = 0; i < word.length; ++i) {
    ...
    if (!curr.links[j]) curr.links[j] = new WordDictionary();
    curr = curr.links[j];
  }
  curr.isWord = true;
};
...
WordDictionary.prototype.search = function (word) {
  ...
  var recDFSHelper = function (node, partWord) {
    if (partWord.length === 0) return node.isWord;
    let code = partWord.charCodeAt(0);
    if (code === 46) {
      // wildcard '.'
      for (let j = 0, len = node.links.length; j < len; ++j) {
        if (node.links[j] && recDFSHelper(node.links[j], partWord.substring(1)))
          return true;
      }
      return false;
    } else {
      return (
        node.links[code - 97] !== null &&
