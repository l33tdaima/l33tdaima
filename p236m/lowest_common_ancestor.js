...
var lowestCommonAncestorSpeedup = function (root, p, q) {
  ...
  var recFindNode = function (curr, p, q) {
    ...
    if (foundInLeft === 2) {
      // already found in the left child
      return 2;
    }
    let foundInRight = recFindNode(curr.right, p, q);
    if (foundInRight === 2) {
      // already found in the right child
      return 2;
    }
...
var buildTestCase = function (data, pNum, qNum) {
  ...
  return { root: rtNode, p: pNode, q: qNode };
};
...
[
  ...
  console.log(
    ...
    pqPair[1],
    '->',
    lowestCommonAncestor(testObj.root, testObj.p, testObj.q).val
  );
});
testData = '#';
