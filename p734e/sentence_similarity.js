...
var areSentencesSimilarV2 = function(words1, words2, pairs) {
  if (words1.length !== words2.length) return false;
  const dict = new Set();
  for (let p of pairs) {
    dict.add(p[0] + "-" + p[1]);
  ...
  for (let i = 0; i < words1.length; ++i) {
    if (
      words1[i] !== words2[i] &&
      !dict.has(words1[i] + "-" + words2[i]) &&
      !dict.has(words2[i] + "-" + words1[i])
    )
      return false;
  }
  return true;
...
[
  ...
  let actual1 = areSentencesSimilar(t.words1, t.words2, t.pairs);
  let actual2 = areSentencesSimilarV2(t.words1, t.words2, t.pairs);
  console.log(
    "",
    t.words1,
    "is similar to",
  ...
  console.assert(actual1 === t.expected);
  console.assert(actual2 === t.expected);
});
